...
if __name__ == '__main__':
    ...
    for frag in common_frags:
        ...
        for site in common_sites:
            ...
            site_t_score = t_score([meth_levels[tw][site] for tw in GAY_TWINS],[meth_levels[tw][site] for tw in HETERO_TWINS])
        ...
        for rand_i in xrange(300):
            ...
            rt_scores = [t_score([meth_levels[tw][site] for tw in GAY_TWINS], [meth_levels[tw][site] for tw in HETERO_TWINS]) for site in common_sites]
            ...
            rt_bar_sigma.append((mean(rt_scores), std(rt_scores)))
    ...
    fdr = FDR(z_scores, alpha = 0.05)
    print 'FDR:', fdr
    accepted = sorted([z for z in z_scores if z['pval'] <= fdr], key = lambda x: abs(x['z_score']), reverse = True)
    print 'Accepted:', len(accepted)
    json.dump(accepted,
              open(os.path.join(DATA_DIR, 'fragments_related_to_sexual_orientation_z_scores.out'), 'w'),
              indent = 2)
