...
#        res = cc(sites, data, [1 if tw in GAY_TWINS else 0 for tw in TWINS if tw != testTwin], testTwin = testTwin)
        res = cc(sites, data, [twin_random[tw] for tw in TWINS if tw != testTwin], testTwin = testTwin)

#        fdr = FDR(res, alpha = alpha)

#        result[testTwin] = [s for s in sorted(res, key = lambda k: res[k]['score'], reverse = True) if res[s].get('pval', -1) <= fdr]

        result[testTwin] = [s for s in sorted(res, key = lambda k: abs(res[k]['score']), reverse = True)][:100]

#        print testTwin , 'FDR:', fdr, 'Accepted:', len(result[testTwin])

        elapsed(testTwin)

    json.dump(result, open(os.path.join(DATA_DIR, 'fragments_for_gay_prediction_top100.out'), 'w'))
